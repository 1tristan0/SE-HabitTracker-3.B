...
*** link:../../src/backend/index.js[index.js] – Express-Server, bindet Auth-/Habit-Routen ein.
...
*** link:../../src/backend/src/routes[ routes] – REST-API (`/api/auth`, `/api/habits`) + Auth-Middleware gegen Supabase.
** link:../../src/frontend[frontend] – React (Create React App).
*** link:../../src/frontend/package.json[package.json] – CRA-Skripte, Bootstrap, Tests mit react-scripts.
*** link:../../src/frontend/tailwind.config.js[tailwind.config.js] – minimale Tailwind-Konfiguration (derzeit kaum genutzt).
*** link:../../src/frontend/src/api/httpClient.js[api/httpClient.js] – HTTP-Wrapper zum Backend (Bearer-Token).
*** link:../../src/frontend/src/api/authApi.js[api/authApi.js] – Auth-Funktionen (Login, Register, Logout, Session) via Backend.
*** link:../../src/frontend/src/api/habitsApi.js[api/habitsApi.js] – CRUD/Toggle auf `habits_table` via Backend.
...
* Auth/Token-Handling: Backend validiert jeden Request live gegen Supabase (`/auth/v1/user`), aber es gibt kein Refresh-Handling/Token-Rotation und keine Rate-Limits.
* Fehlerbehandlung/Logging: Express-Routen haben kein zentrales Error-Handling, keine Health/Ready-Checks.
* Prisma-Client-Pfad: Output im Quellordner (`src/generated/prisma`) statt im Standard `.prisma/client` → unüblich, aber funktional.
* Env-Handling: Supabase-Keys gehören ins Backend, aber `.env.example` fehlt weiterhin; DB-URL liegt unverschlüsselt im Repo.
...
1. `.env.example` nachziehen (Backend: `DATABASE_URL`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`; Frontend: `REACT_APP_API_BASE_URL`).
2. Token-Handling härten (Refresh/Rotation, Caching von User-Infos statt pro Request gegen Supabase zu callen).
3. Tests ergänzen (Backend: Supertest für `/api/auth` und `/api/habits`; Frontend: RTL-Mocks für API-Client).
4. Optional: React Router für Navigation, Volume-Mounts im Docker-Dev für Hot-Reload.
