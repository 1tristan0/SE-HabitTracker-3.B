...
. **Nutzung von Supabase (PostgreSQL) und Prisma ORM**  
- Spart Zeit durch fertige Authentifizierungslösungen und Datenbankverwaltung in der Cloud. 
- Team muss keine eigene Lösung zum Hosting einer Datenbank finden.
- Verwendung eines ORM ermöglicht komplexere Features mit geringerem Aufwand umzusetzen sowie kein reines SQL im Backend verwalten zu müssen
- Prisma Schema verringert den Aufwand, Anpassungen des Domänenmodells auf DB-Schema zu übertragen (Objektorientiert)
...
• DB-Abfragen/Operationen werden durch den Prisma Client ausgeführt
...
. **Kommunikation:**  
  ...
  – Prisma Client verknüpft Logik- und Datenhaltungsschicht  
...
....
==== Erklärung:
Unsere App {project-system-name} ist ein einfacher Habit-Tracker:
...
[plantuml, "{diagramsdir}/c4-level3", svg]
....
include::{plantumlsdir}/c4-level3.puml[]
....
==== Erklärung:
* **C3-Diagramm (Komponentensicht)**:  
  Zeigt die wichtigsten Komponenten innerhalb des **Backend API**-Containers und deren Zusammenspiel.

* **Controller/Routen**:  
  Endpunkte der REST-API, die HTTP-Requests entgegennehmen und an die Geschäftslogik weiterleiten.

* **Services**:  
  Kapseln die Geschäftslogik (Use Cases) und koordinieren die Verarbeitung.

* **Repositorys/Adapter (Prisma Client)**:  
  Verantwortlich für den Datenzugriff auf die Supabase/PostgreSQL-Datenbank.

* **Querschnittsfunktionen**:  
  Authentifizierung (Supabase Auth), Validierung und Fehlerbehandlung sind klar sichtbar.

* **Abhängigkeitsrichtung**:  
  UI → API → Service → Datenzugriff.
...
....
include::{plantumlsdir}/deployment.puml[]
....
==== Erklärung:
* **Nutzergerät (Browser)**:  
  Öffnet das Frontend im Browser und kommuniziert über HTTP/REST.

* **Docker Host**:  
  Führt die Container für Frontend (React/CRA), Backend (Node/Express) und die lokale Supabase-Instanz aus.

* **Supabase Local (Dev)**:  
  Stellt Auth/API, PostgreSQL und Studio lokal bereit (Ports 54321-54323).

* **Supabase Cloud (Prod)**:  
  Externer Dienst für Authentifizierung und PostgreSQL-Datenhaltung in der Produktionsumgebung.

* **Kommunikation**:  
  Browser → Frontend → Backend per HTTP/REST; Backend → Supabase lokal (Dev) bzw. Supabase Cloud (Prod).

=== Sequenzdiagramme
//Beschreibt die Interaktionen zwischen Objekten in einem bestimmten Szenario einer Funktionalität des Systems. Diese Sicht wird verwendet, um die Zusammenarbeit zwischen Objekten zu verdeutlichen und den Ablauf von Nachrichten und Methodenaufrufen zu zeigen.
Sequenzdiagramm Habit anlegen
[plantuml, "{diagramsdir}/sequenz_habit_anlegen", svg]
....
include::{plantumlsdir}/sequenz_habit_anlegen.puml[]
....
Sequenzdiagramm Habit abhaken
[plantuml, "{diagramsdir}/sequenz_habit_abhaken", svg]
....
include::{plantumlsdir}/sequenz_habit_abhaken.puml[]
....
Sequenzdiagramm Habit bearbeiten
[plantuml, "{diagramsdir}/sequenz_habit_bearbeiten", svg]
....
include::{plantumlsdir}/sequenz_habit_bearbeiten.puml[]
....
Sequenzdiagramm User registrieren
[plantuml, "{diagramsdir}/user_registration", svg]
....
include::{plantumlsdir}/user_registration.puml[]
....
Sequenzdiagramm user log in
[plantuml, "{diagramsdir}/user_login", svg]
....
include::{plantumlsdir}/user_login.puml[]
....
